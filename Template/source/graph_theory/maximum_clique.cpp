...
struct Maxclique {
    ...
    void color_sort(Vertices &R) {
        int j = 0, maxno = 1;
        int min_k = max((int) QMAX.size() - (int) Q.size() + 1, 1);
        C[1].clear(), C[2].clear();
    ...
    void mcqdyn(int *maxclique, int &sz) {
        set_degrees(V);
        sort(V.begin(), V.end(), desc_degree);
        init_colors(V);
    ...
    void degree_sort(Vertices &R) {
        set_degrees(R);
        sort(R.begin(), R.end(), desc_degree);
    }

    Maxclique(const BB *conn, const int sz, const float tt = .025)
