...
bool trans(const point<int64_t> a, double rotate, double scale, point<int64_t> &ret) {
...
bool judge(const point<int64_t> delta, double rotate, double scale) {
    //printf("case (%ld, %ld) %ld %ld\n", delta.x, delta.y, rotate, scale);
    set.clear();
    for (auto i : b) set.insert(i);
    for (auto i : a) {
        line<int64_t> l;
        if (!trans(i.a, rotate, scale, l.a) ||
                !trans(i.b, rotate, scale, l.b))
            return false;
        l.a = l.a + delta;
        l.b = l.b + delta;
        pure_line(l);
