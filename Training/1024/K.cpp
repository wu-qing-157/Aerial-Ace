...
int main()
{
	...
	dfs(0, 0); mp[1] = 1;
	pw[0] = 1;
	for (int i = 1; i <= 100; ++i) pw[i] = 100ll * pw[i - 1] % mod;
	for (int i = 2; i <= 100; ++i)
	{
	...
	for (; T; --T)
	{
		...
		for (int i = 1, j; i <= n; lst = i, i = j)
		{
			...
			if (mp[now] < 10)
			{
				...
				{
					fa = fd[S];
					f[i][S] = (f[i][S] + 1ll * f[i - 1][S] * (100 - p[now])) % mod;
					int ct = con[S];
			...
			else
			{
				...
				for (int ii = i + 1; ii < j; ++ii)
					pp = 1ll * pp * (100 - p[r[ii]]) % mod;
				ans = (ans + 1ll * pp * pw[n - (j - i)]) % mod;
				for (int S = 1; S <= cnter; ++S)
				{
					for (int SS = 0, _ = j - i; SS < (1 << _); ++SS)
					{
						...
						for (int ii = 0; ii < _; ++ii)
							...
							else pp = 1ll * pp * (100 - p[r[ii + i]]) % mod;
						for (int ii = 0; ii < 4; ++ii)
							if (tot % pr[ii + 1] == 0 && fa[ii] == 0)
								fa[ii] = ++ct;

						for (int i2 = 0; i2 < 4; ++i2)
						for (int jj = i2 + 1; jj < 4; ++jj)
							if (tot % pr[i2 + 1] == 0 && tot % pr[jj + 1] == 0)
							{
